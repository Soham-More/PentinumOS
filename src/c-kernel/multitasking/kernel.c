...
struct {
    ...
    kthread_info_t threads[MAX_KERNEL_THREADS];
    usize thread_count;
...
void kmt_restore_flags(u32* flags) { g_kmt_ctx.flags = *flags; }

// code to push to the queue, expects no PREEMPTION, and caller should ensure thread_id is valid
void kmt_queue_push(kmt_queue_t* queue, kthread_uid_t thread_id) {
...
// code to push to the queue, expects no PREEMPTION, and caller should ensure thread_id is valid
kthread_uid_t kmt_queue_pop(kmt_queue_t* queue) {
    if(!queue->head) return KMT_INVALID_KTHREAD_UID;

    kmt_queue_node_t* node = queue->head;
    kthread_uid_t thread_id = node->thread_id;
...
// if keep_ready is true, the current thread will be put back to the ready queue
void kmt_schedule(bool keep_ready) {
    STOP_PREEMPTING();

    // retire the current thread if it's not already terminated
    if(g_kmt_ctx.tcb_pool[g_kmt_ctx.current_thread].status != KTHREAD_STATUS_TERMINATED) {
        g_kmt_ctx.tcb_pool[g_kmt_ctx.current_thread].status = KTHREAD_STATUS_IDLE;
        // if the thread is not the idle thread, we put it back to the ready queue
        if(keep_ready && g_kmt_ctx.current_thread != g_kmt_ctx.idle_thread) {
    ...
    kthread_uid_t next_thread_id = kmt_queue_pop(&g_kmt_ctx.ready_queues[max_priority]);
    ...
    if(g_kmt_ctx.tcb_pool[next_thread_id].status != KTHREAD_STATUS_READY) {
        panic(
    ...
    g_kmt_ctx.tcb_pool[next_thread_id].status = KTHREAD_STATUS_RUNNING;
...
void _asmcall kmt_thread_start(thread_entry_point_t entry_point) {
    ...
    g_kmt_ctx.tcb_pool[g_kmt_ctx.current_thread].status = KTHREAD_STATUS_TERMINATED;
    kmt_schedule(false);
}
...
void kmt_preemptive_intr_handler(registers_t* registers) {
    ...
    while(g_kmt_ctx.sleep_heap_size > 0 && g_kmt_ctx.sleep_heap[0].wakeup_time <= current_time) {
        kthread_uid_t thread_id = g_kmt_ctx.sleep_heap[0].thread_id;
        kmt_pop_sleep_heap();
    ...
    while(ready_bitmap > 1) {
        ...
        while(node) {
            ...
            if(g_kmt_ctx.tcb_pool[node->thread_id].age >= KMT_AGE_TICKS) {
                ...
                g_kmt_ctx.tcb_pool[node->thread_id].status = KTHREAD_STATUS_IDLE;
...
kthread_uid_t kmt_create_thread(const kthread_desc_t *desc) {
    ...
    kthread_uid_t new_thread_id = g_kmt_ctx.thread_count;
	...
	*stack_esp = (u32)desc->stack_top; // ebp is the stack top (for entry point)
    ...
    g_kmt_ctx.tcb_pool[new_thread_id] = (ktcb_t){
        .esp = stack_esp,
        .esp0 = desc->interrupt_stack_top,
        .cr3 = x86_get_ctx_map(&desc->ptable),
        .status = KTHREAD_STATUS_IDLE,
...
err_t kmt_spawn_idle_thread(const kthread_desc_t* desc) {
    ...
    if(g_kmt_ctx.idle_thread != invalid_u16) return EEXISTS;
    g_kmt_ctx.idle_thread = kmt_create_thread(desc);
    if(KMT_IS_INVALID_UID(g_kmt_ctx.idle_thread)) {
        u32 err_code = KMT_GET_ERR_UID(g_kmt_ctx.idle_thread);
        g_kmt_ctx.idle_thread = invalid_u16;
        return err_code;
    }
...
err_t kmt_wakeup_thread(kthread_uid_t thread_id) {
    ...
    if(g_kmt_ctx.tcb_pool[thread_id].status == KTHREAD_STATUS_READY) return ESUCCESS;
    else if(g_kmt_ctx.tcb_pool[thread_id].status == KTHREAD_STATUS_RUNNING) return ESUCCESS;
    else if(g_kmt_ctx.tcb_pool[thread_id].status == KTHREAD_STATUS_TERMINATED) return ETERMINATED;
    ...
    g_kmt_ctx.tcb_pool[thread_id].status = KTHREAD_STATUS_READY;
...
err_t kmt_unlock_mutex(kthread_mutex_t mutex) {
    ...
    kmt_mutex_impl_t* mutex_impl = &g_kmt_ctx.mutex_pool[mutex];
    if(mutex_impl->flags == KMT_MUTEX_FREE) return EUSEFREED;
    if(mutex_impl->owner != g_kmt_ctx.current_thread) return EINVAL;
    if(mutex_impl->waiting_queue_head) {
        // wake up the next thread in the waiting queue
        kmt_queue_node_t* next_thread_node = mutex_impl->waiting_queue_head;
        ...
        kmt_wakeup_thread(next_thread_node->thread_id);
...
// thread info
kthread_uid_t kmt_get_current_thread() { return g_kmt_ctx.current_thread; }
...
heap_allocator_t* kmt_get_heap() {
    kthread_uid_t thread_id = g_kmt_ctx.current_thread;
