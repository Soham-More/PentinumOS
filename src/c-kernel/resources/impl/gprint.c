...
void gp_putcstr(void* ctx, const char* cstr) {
    while(*cstr) {
        gp_putc(ctx, *cstr);
        cstr++;
    }
}
void gp_putstr(void* ctx, const char* cstr, usize len) {
...
void gp_print_formatted_string(void* ctx, const char* str, usize len, gp_fmt_spec spec) {
    // spec.width, spec.align, spec.fill
    usize padding = spec.width > len ? spec.width - len : 0;
    if(spec.align == '>') {
        for(usize i = 0; i < padding; i++) {
            gp_putc(ctx, spec.fill);
        }
    }
    gp_putstr(ctx, str, len);
    if(spec.align == '<') {
        for(usize i = 0; i < padding; i++) {
            gp_putc(ctx, spec.fill);
        }
...
void gp_print_spec(void* ctx, gp_fmt_spec spec, va_list* vargs) {
    // print variable according to spec
    if(spec.error != GP_OK) {
        gp_putc(ctx, '\n');
        gp_putcstr(ctx, "[gp_printer] invalid format specifier: ");
        gp_putcstr(ctx, gp_error_strings[spec.error]);
        gp_putc(ctx, '\n');
        return;
    }
    ...
    case 's':
        ...
        gp_print_formatted_string(ctx, str, gp_cstr_len(str), spec);
        return;
    case 'S':
        // print string of length
        str = va_arg(*vargs, const char*);
        len = va_arg(*vargs, usize);
        gp_print_formatted_string(ctx, str, len, spec);
        return;
    case 'c':
        // print char
        char ch = (char)va_arg(*vargs, int);
        gp_print_formatted_string(ctx, &ch, 1, spec);
        return;
    case 'x':
    case 'X':
    case 'u':
        ...
        if(spec.bytesize <= 32) {
            ...
            gp_print_formatted_string(ctx, (const char*)buf, len, spec);
    ...
    case 'p':
        ...
        }
        {
            ...
            gp_print_formatted_string(ctx, (const char*)big_buf, 2 + zero_padding + len, spec);
        }
        return;
    case 'v':
        // TODO: bitfield view
    ...
    for(usize i = 0; i < len; i++) {
        if(in_spec) {
            if(fmt[i] == '{') {
                if(spec_start - i > 0) {
                    gp_putc(ctx, '\n');
                    gp_putcstr(ctx, "[gp_printer] invalid format specifier: unexpected '{' in specifier\n");
                    return;
                }
                gp_putc(ctx, '{');
                in_spec = false;
            ...
            gp_putc(ctx, '\n');
            gp_putcstr(ctx, "[gp_printer] invalid format specifier: unexpected '}'\n");
            return;
        } else {
            gp_putc(ctx, fmt[i]);
        }
...
void gp_print(void* ctx, const char* fmt, ...) {
    va_list vargs;
    va_start(vargs, fmt);
    gp_printer(ctx, fmt, gp_cstr_len(fmt), &vargs);
    va_end(vargs);
}
