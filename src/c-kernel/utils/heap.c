...
#define HEAP_DEFAULT_ALIGN 0x1
u32 get_aligned_size(u32 req_size, u32 align)
{
...
heap_allocator_t* initialize_heap(heap_allocator_t* global_allocator, u8* pool, usize pool_size_bytes)
{
    return construct_heap(
        malloc(global_allocator, get_heap_allocator_bytesize()),
        pool,
        pool_size_bytes
    );
}
...
heap_allocator_t* construct_heap(void* memory, u8* pool, usize pool_size_bytes) {
    heap_allocator_t* heap_allocator = (heap_allocator_t*)memory;
    heap_allocator->heap_bin_top = pool;
    heap_allocator->heap_bin_head = 0;
    heap_allocator->heap_bin_size = pool_size_bytes;
    heap_allocator->alloc_bin = construct_heap_bin();
    heap_allocator->free_bin = construct_heap_bin();
    return heap_allocator;
}
// function to get the size of heap_allocator_t
usize get_heap_allocator_bytesize() { return sizeof(heap_allocator_t); }
...
void* malloc_aligned(heap_allocator_t* heap_allocator, u32 req_size, u32 align)
{
    ...
    align = ceil_to_2power(align);
    // if normal alignment
    if(align <= 0x10) return malloc(heap_allocator, req_size);

    // otherwise, find out padding
    u32 heap_head_addr = (u32)(heap_allocator->heap_bin_top + heap_allocator->heap_bin_head);
    ...
    if(padding == 0) return malloc(heap_allocator, req_size);

    // make sure there is enough space
    if(heap_allocator->heap_bin_size < padding + get_aligned_size(req_size, HEAP_DEFAULT_ALIGN))
    {
        //x86_raise(NO_HEAP_MEMORY);
        return nullptr;
    }

    heap_node* padding_node = (heap_node*)(heap_allocator->heap_bin_top + heap_allocator->heap_bin_head);
    *padding_node = construct_heap_node(padding - sizeof(heap_node));
    
    // add the padding node to free bin
    padding_node->flags |= HEAP_FREE_NODE;
    bin_push(&heap_allocator->free_bin, padding_node);

    heap_allocator->heap_bin_head += padding;
    heap_allocator->heap_bin_size -= padding;

    // return normal malloc
    return malloc(heap_allocator, req_size);
}
void* malloc(heap_allocator_t* heap_allocator, u32 req_size)
{
    ...
    if (req_node_size <= heap_allocator->heap_bin_size)
    {
        heap_node* node = (heap_node*)(heap_allocator->heap_bin_top + heap_allocator->heap_bin_head);
        *node = construct_heap_node(aligned_req_size);

        bin_push(&heap_allocator->alloc_bin, node);

        heap_allocator->heap_bin_head += req_node_size;
        heap_allocator->heap_bin_size -= req_node_size;
    ...
    heap_node* node = bin_find_node(heap_allocator->free_bin, aligned_req_size + sizeof(heap_node) + MIN_NODE_SIZE);

    // there was nothing big enough
    if (node == nullptr)
    {
        node = bin_find_node(heap_allocator->free_bin, aligned_req_size);
        // out of memory
        if(node == nullptr)
        {
            //x86_raise(NO_HEAP_MEMORY);
        ...
        bin_remove(&heap_allocator->free_bin, node);
        bin_push(&heap_allocator->alloc_bin, node);
    ...
    *split_node = construct_heap_node(node->size - aligned_req_size - sizeof(heap_node));
    ...
    *node = construct_heap_node(aligned_req_size);
    ...
    bin_push(&heap_allocator->free_bin, split_node);
...
void* realloc(heap_allocator_t* heap_allocator, void *prev_ptr, u32 new_req_size)
{
    ...
    u32 node_end_pos = (u32)(prev_ptr) - (u32)(heap_allocator->heap_bin_top) + node->size;
    if(node_end_pos == heap_allocator->heap_bin_head)
    {
        heap_allocator->heap_bin_head += aligned_req_size - node->size;
        heap_allocator->heap_bin_size -= aligned_req_size - node->size;
    ...
    if(node->flags & HEAP_HINT_RESIZEABLE)
    {
        ...
        if(next_node->size + sizeof(heap_node) >= (aligned_req_size - node->size) + sizeof(heap_node) + MIN_NODE_SIZE)
        {
            ...
            bin_remove(&heap_allocator->free_bin, next_node);

            // yes, so now split it
            // split the node
            heap_node* split_node = (heap_node*)((u8*)next_node + split_offset);
            *split_node = construct_heap_node(next_node->size - split_size);
    ...
    // nothing is possible :(
    void* new_ptr = malloc(heap_allocator, new_req_size);
    memcpy(new_ptr, prev_ptr, new_req_size);
    free(heap_allocator, prev_ptr);
...
{
    void* heap_obj = malloc(heap_allocator, bytelength);
    if(IS_INV_PTR(heap_obj)) return heap_obj;
    return memcpy(heap_obj, obj, bytelength);
}

void log_heap_status(heap_allocator_t *heap_allocator)
{
    log_debug("Heap: Bin(loc = {p}, pos = {p}, free = {p})\n", heap_allocator->heap_bin_top, heap_allocator->heap_bin_head, heap_allocator->heap_bin_size);

    log_debug("\tAllocated Bin:\n");
    bin_log(heap_allocator->alloc_bin, "\t\t", 0);
    log_debug("\tFree Bin:\n");
    bin_log(heap_allocator->free_bin, "\t\t", 0);
}
...
void free(heap_allocator_t* heap_allocator, void* ptr)
{
    ...
    bin_remove(&heap_allocator->alloc_bin, node);

    // wait let's see if the end of this heap
    // is the position of the top of the heap
    u32 node_end_pos = (u32)(ptr) - (u32)(heap_allocator->heap_bin_top) + node->size;
    
    //if(true)
    if(node_end_pos != heap_allocator->heap_bin_head)
    {
        // more likely
        bin_push(&heap_allocator->free_bin, node);
    }
    else
    {
        // damn! then we will just merge it
        heap_allocator->heap_bin_head -= sizeof(heap_node) + node->size;
        heap_allocator->heap_bin_size += sizeof(heap_node) + node->size;
